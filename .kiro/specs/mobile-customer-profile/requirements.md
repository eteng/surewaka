...
The customer mobile app's profile tab shows identity and preference data sourced entirely from `public.users` (a single DB fetch via the API layer — no API layer). Customers can edit their name, email, and gender directly. Email changes go through Clerk verification; a Clerk webhook syncs the confirmed email back to `public.users`. Notification preferences are persisted to the same row.
...
- **Pending_Email**: An email submitted via `clerk.updateUser({ email })` that has not yet been confirmed. Visible in the Clerk user object as `user.externalAccounts`.
...
1. WHEN the profile tab mounts, THEN the Profile_Hook SHALL fetch the Customer_Profile row from `public.users` using the Hono API with the Clerk user's JWT.
2. THE profile tab SHALL display name, phone, email, and gender sourced from `public.users` — not from Clerk session metadata.
...
1. WHEN a customer submits a new name, THEN the Profile_Hook SHALL update `public.users.name` via `db.query('users').update(...)` scoped to their own `id`.
2. WHEN the name update succeeds, THEN the Profile_Hook SHALL also call `clerk.updateUser({ data: { name } })` to keep auth metadata consistent.
...
1. WHEN a customer submits a new email, THEN the Profile_Hook SHALL call `clerk.updateUser({ email })` to initiate the Email_Verification_Flow.
...
3. THE pending state SHALL be derived from `user externalAccounts in Clerk — no extra DB query.
...
3. WHEN a customer selects a gender and saves, THEN the Profile_Hook SHALL update `public.users.gender` via the API layer.
...
2. WHEN a customer toggles a notification switch, THEN the Profile_Hook SHALL immediately update the corresponding column in `public.users` via the API layer.
...
3. THE trigger SHALL be an AFTER UPDATE trigger to avoid interfering with Clerk's own write.
