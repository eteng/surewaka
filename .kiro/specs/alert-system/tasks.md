...
- [x] 1. Database schema — alerts table, alert_settings, push_tokens extension
  - [x] 1.1 Create Drizzle schema file `packages/db/src/schema/alerts.ts`
  ...
  - [x] 1.2 Create Drizzle schema file `packages/db/src/schema/alert-settings.ts`
  ...
  - [x] 1.3 Update `push_tokens` schema to extend `app` enum/constraint to include `'admin'`
...
- [x] 2. Shared types — AlertRule, AlertSeverity, Alert, constants, validators
  - [x] 2.1 Add alert constants to `packages/shared/src/constants.ts`
  ...
  - [x] 2.2 Add alert types to `packages/shared/src/types.ts`
  ...
  - [x] 2.3 Add `updateAlertSettingsSchema` validator to `packages/shared/src/validators.ts`
...
- [x] 3. Checkpoint — Shared types compile and validate
  - Ensure `pnpm --filter @surewaka/shared build` passes with no errors, ask the user if questions arise.

- [x] 4. Alert engine worker scaffold
  - [x] 4.1 Create worker package structure
  ...
  - [x] 4.2 Create worker-local types and DB client
...
- [x] 5. Alert engine core modules — settings loader, Pumble sender, admin push enqueuer
  - [x] 5.1 Create settings loader `workers/alert-engine/src/settings.ts`
  ...
  - [x] 5.2 Create Pumble webhook sender `workers/alert-engine/src/pumble.ts`
  ...
  - [x] 5.3 Create admin push enqueuer `workers/alert-engine/src/push.ts`
...
- [x] 6. Alert rules — all 7 evaluation rules
  - [x] 6.1 Implement `driver_silent` rule at `workers/alert-engine/src/rules/driver-silent.ts`
  ...
  - [x] 6.2 Implement `leg_overdue` rule at `workers/alert-engine/src/rules/leg-overdue.ts`
  ...
  - [x] 6.3 Implement `driver_ghost` rule at `workers/alert-engine/src/rules/driver-ghost.ts`
  ...
  - [x] 6.4 Implement `dispute_filed` rule at `workers/alert-engine/src/rules/dispute-filed.ts`
  ...
  - [x] 6.5 Implement `delivery_failed` rule at `workers/alert-engine/src/rules/delivery-failed.ts`
  ...
  - [x] 6.6 Implement `ontime_rate_drop` rule at `workers/alert-engine/src/rules/ontime-rate-drop.ts`
  ...
  - [x] 6.7 Implement `customer_update_gap` rule at `workers/alert-engine/src/rules/customer-update-gap.ts`
...
- [x] 7. Checkpoint — Alert rules compile and unit tests pass
  - Ensure `pnpm --filter @surewaka/alert-engine test` passes, ask the user if questions arise.

- [x] 8. Alert engine main polling loop and lifecycle manager
  - [x] 8.1 Implement `workers/alert-engine/src/index.ts` — polling loop entry point
  ...
  - [x] 8.2 Implement `upsertAlert()` function in `workers/alert-engine/src/index.ts`
  ...
  - [x] 8.3 Implement `routeCritical()` function in `workers/alert-engine/src/index.ts`
...
- [x] 9. Checkpoint — Alert engine runs and processes alerts
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Alert settings API route
  - [x] 10.1 Create `apps/api/src/routes/admin/alert-settings.ts`
  ...
  - [x] 10.2 Register alert-settings route in `apps/api/src/index.ts`
...
- [x] 11. Alert settings UI — `/settings/alerts` page
  - [x] 11.1 Create data fetching hook `apps/admin/app/hooks/use-alert-settings.ts`
  ...
  - [x] 11.2 Create settings page `apps/admin/app/routes/settings/alerts.tsx`
  ...
  - [x] 11.3 Add navigation link to alerts settings from parent settings layout
