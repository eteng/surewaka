...
export function useSystemConfig() {
  ...
  const saveConfig = useCallback(
    async (key: string, value: unknown): Promise<boolean> => {
      ...
      try {
        ...
        setItems((prev) =>
          prev.map((item) =>
            item.key === key ? { ...item, value, updatedAt: json.data.updatedAt } : item,
          ),
        );
        setSaveSuccess(key);
    ...
    async (key: string): Promise<boolean> => {
      ...
      try {
        ...
        setItems((prev) =>
          prev.map((item) =>
            item.key === key
              ? { ...item, value: json.data.value, updatedAt: null, updatedBy: null }
              : item,
          ),
        );
        setSaveSuccess(key);
  ...
  const importConfig = useCallback(
    async (file: File): Promise<{ imported: number; skipped: number } | null> => {
      const token = await getToken();
