...
import { columns } from '~/components/drivers/driver-columns';
...
export function DriverDataTable({
  ...
  return (
    <div className="overflow-x-auto rounded-md border">
      <table className="w-full text-sm" role="grid" aria-label="Driver listing">
        <thead>
          ...
          {isLoading ? (
            ...
            table.getRowModel().rows.map((row) => (
              <tr
                ...
                tabIndex={0}
                role="row"
                aria-label={`Driver: ${row.original.name}`}
              >
